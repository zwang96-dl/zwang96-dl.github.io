...
> **本章要点(TL;DR)**
>
> - 上一章那个流水线 GEMM,还是把「加载 / 计算 / 写回」三件事全压在一个 warpgroup 上,三个硬件引擎(TMA、Tensor Core、写回路径)只能轮流空转。本章用 Step 7–9 三步,一级一级把「谁跟谁协作」的范围撑大,一路逼近业界最高的吞吐。
...
> **前置知识**:读这一章前,最好先懂 warp 专门化、cluster(集群)、TMEM(Tensor Core 专用累加器内存)、mbarrier/phase(屏障与相位)这几样,它们是本章三步优化的地基。没把握的话,先翻一下 [第 0 章 · 极简入门](./ch00_gpu_ml_primer.md),以及本章直接接力的第 11–12 章(TMA 流水线、`tcgen05` MMA、TMEM 累加器的契约都来自那里)。本章会默认你已经认识这些词。

## 引言:用 Warp 专门化与 Cluster 把 GEMM 推到极致(Scaling GEMM with Warp Specialization and Clusters)

### 上一章遗留的瓶颈:一个 warpgroup 包揽所有事

上一章我们用 TMA(张量内存加速器 / Tensor Memory Accelerator)把 GEMM(矩阵乘法)做成了流水线,跑得已经挺快了。可它骨子里有个毛病。

毛病出在哪?整条流水线就只有**一个 warpgroup**(4 个 warp、128 个线程凑成的一支队伍;一个 warp / 线程束 = 32 个线程的小班,见第 0 章),却要它一个人按顺序干完三件压根不是一回事的活——

1. **发起搬运(load)**:用 TMA 把下一块输入 tile(大矩阵切出来的小方块)从全局内存搬进共享内存(SMEM)。
2. **跑矩阵乘(MMA)**:让 Tensor Core(专做矩阵乘累加的硬件单元,见第 0 章)在这块数据上做矩阵乘累加。
3. **写回(writeback)**:把累加结果从 TMEM(Tensor Memory,Tensor Core 专用的累加器内存)排空,写回内存。

就算用软件流水线把这三步在时间上叠了一点,根子上的问题还在:**三种硬件引擎(TMA 单元、Tensor Core、写回路径)全堵在同一队线程身上**。这队线程就成了三个引擎的独木桥,谁想干活都得排队过,自然谁也快不起来。
...
> **注意**:这里的 CTA(Cooperative Thread Array,即一个 thread block)是一个线程块;cluster 则是 Hopper/Blackwell 架构引入的、把多个 CTA 绑成一组、允许它们互相访问对方 SMEM 的更上层结构。
...
> **注意**:Step 8 是协作范围头一次伸到单个 CTA 之外。也正因为这样,它的操作数(operand,即喂给矩阵乘的输入矩阵 A、B)tile 被**劈成两半,分别放进两个 CTA 的共享内存**,布局沿着 cluster 的 `cbx` 轴(也就是 cluster 在 x 方向上给 CTA 的编号)**横跨两个 CTA**。读 Step 8 的代码时,脑子里一定得绷着这根弦:同一个逻辑 tile,物理上是散在两个 CTA 的 SMEM 里的。
## 第 7 步:Warp 专门化 + 流水线(Step 7: Warp Specialization + Pipeline)

到目前为止,我们的 GEMM 内核都是「单 warpgroup」结构:同一批线程顺着「加载 → 计算 → 写回」走一遍。这写法有处要命的浪费——线程在搬数据(TMA)的时候,Tensor Core 干瞪眼;线程在跑 MMA 的时候,TMA 引擎又闲着。两个金贵的硬件引擎,愣是凑不到一块儿同时忙。

这一步的核心就一个词:**warp 专门化 / warp specialization**。它要治的病是这样的——与其让一支队伍轮着把所有活都干了,不如一件活配一支**专门的 warp**,让这些 warp **一起跑**,再拿一套软件流水线把它们串起来。这是整条 GEMM 路上动静最大的一次架构改造,后面第 8、9 步全是踩着它的肩膀往上走的。本节基准统一用 `M=N=K=4096`。
...
- **`tma2mma`**(TMA → MMA):跟 MMA 说「你等的那片 SMEM 数据到位了,开算吧」。
- **`mma2tma`**(MMA → TMA):跟 TMA 说「这块缓冲我读完了,你拿去装下一片」。

> **注意**:时间线里有个细节乍一看像 bug——`mma2tma` 的释放箭头会「跨一个 stage」。别慌,这是**环形缓冲(ring buffer)**的正常现象。`PIPE_DEPTH=2` 的时候只有两块 SMEM 缓冲(stage 0、stage 1):TMA 把 k=0 装满 buffer 0,k=1 装满 buffer 1。等 MMA 算完 k=0、读完 buffer 0,它就发个 `mma2tma`,意思是「buffer 0 空了」。可这会儿真正惦记着 buffer 0 的是谁?是 **k=2**——因为 buffer 1 还被 k=1 占着呢。所以这个释放箭头会一路指到 k=2。说白了,跨一个 stage 纯粹是因为环上就两个槽,转一圈正好回到自己。
...
**为什么每个 barrier 的类型不一样?** 因为类型取决于「生产者拿什么方式告诉别人自己干完了」:

- **TMA 加载**用 `TMABar`,它本质是一个带**字节计数 / byte counting**的 mbarrier:TMA 硬件把该传的字节全搬到位以后,**硬件自己**就在 barrier 上 arrive 了。消费者一行轮询都不用写,就知道数据到了。
- **TMA 存储(store)**使不了这招——store 是把数据往外写,根本没有「该通知谁」这个对象。所以它只能退回到 `cp_async.bulk.commit_group()` + `wait_group(0)`:谁发起的存储,谁自己等自己的写入排空。
- **MMA**用 `TCGen05Bar`,MMA 一算完,由 `tcgen05.commit()` 这条指令在 barrier 上发信号。
...
4 个 barrier 负责告诉各角色**什么时候**某块缓冲就绪了。但还差一样:得有人记着**眼下轮到哪块缓冲**。这份记账活就交给 `PipelineState`。环形缓冲要同时盯两件事:

1. **stage**:现在在第几个槽位;
2. **phase**:现在在等这个槽位 barrier 的哪个「相位」(0 还是 1)。
...
专门化带来一个特别容易踩的同步坑:一旦各个 warpgroup 走的代码路径不一样,我们熟得不能再熟的 `cta_sync()` 就会**死锁**。为啥?因为 `cta_sync()` 用的是硬件 barrier #0,而且死死要求 **CTA 里所有线程**都到齐才放行;可在某个 warpgroup 的分支里,只有一部分线程会走到这,人数永远凑不齐。

我们要的是一个「只管一个 warpgroup」的 barrier。GPU 一共给了 16 个命名 barrier(ID 0–15),所以内核改用 `warpgroup_sync(10)`——它只同步**一个 warpgroup 内部**的线程。要是多个 warpgroup 都得各自独立同步(比如第 9 步的多消费者),它们就用 `warpgroup_sync(wg_id + 10)` 各拿一个不同的 ID,省得撞到同一个硬件 barrier 上去。

### 完整内核实现

这一步用 `PIPE_DEPTH=2`——这是「还能让 load 和 compute 叠起来」的最小深度(更深能多藏点内存延迟,代价是更吃 SMEM;这个取舍后面「当第 7 步出问题」一节细聊)。把前面这些零件——角色、4 个 barrier、`PipelineState`、warpgroup 级同步——拼到一块儿,核心结构长这样(完整源码挺长,这里只留**关键骨架 + 中文讲解**):
...
第 7 步的 epilogue(收尾,即把累加结果转格式、写回内存的最后一段)能写得相当省事:因为只有 `BLK_N=128` 列,写回 warpgroup 一趟就能把整块 TMEM tile 读进寄存器(register,每个线程私有的最快存储,数量有限,见第 0 章),然后发**一条** TMA store 收工。第 8、9 步就没这福气了(这也正是它俩后面非得搞「分块写回」的原因)。第 7 步的写回顺序是这样:
...
> **注意**:第 8、9 步为啥不能也「一趟读完」?卡在**寄存器压力**上。第 8 步 `BLK_N=256`、第 9 步每个消费者 `MMA_N=256`,意味着每个线程得让 256 个 fp32(`256×4=1024` 字节)同时占着寄存器,有溢出(spill)到本地内存的风险,还会逼出更大的 Dsmem 缓冲。所以那两步把写回切成一块 `EPI_N` 列(`EPI_N=64`)一块:每轮迭代只让 `EPI_N` 个 fp32 活在寄存器里,配一条对应的小 TMA store——拿「多发几条 store 指令」换「寄存器预算一直宽裕」。
...
> **注意**:这一步真正赚到的不是「把活儿重新分一遍」,而是实打实地把**算术强度 / arithmetic intensity**——也就是每搬一字节操作数能做多少次计算——给提上来了。下文会专门讲它为什么大约翻了一倍。
...
- **CTA cluster**:多个 CTA 合伙算一个更大的 tile。
- **跨 CTA 读 SMEM**:用 `map_shared_rank` 去读邻居 CTA 的共享内存(也就是 DSMEM,分布式共享内存 / distributed shared memory)。
- **`cta_group=2` 的协同 MMA**:在 256×256 的 cluster tile 上做一次协同矩阵乘。
- **跨 CTA 发 barrier 信号**:用 `cta_mask` 一次把信号发给 cluster 里两个 CTA 的 barrier。
...
搬运量翻 2 倍,计算量翻 4 倍——一除就明白了:每搬一字节操作数,MMA 干的计算量大约翻了倍。凭啥能这样?因为每个 CTA 的 B 切片,通过协同 MMA 被「借」到了对方 CTA 的 A 切片上复用了一遍。换句话说,**算术强度大约翻倍**。对一个还偏内存受限(memory-bound)的 kernel 来讲,这正是最想要的那根杠杆。后面 End-to-End 性能表里那个约 2.2× 的加速,根子就在这儿:同样多的字节,喂了更多的计算。
...
要看懂这一步,关键得抓住整个设计赖以立住的那个**非对称 / asymmetry**:两个 consumer 各用**自己的** A block,去乘**同一块** staged B tile。
